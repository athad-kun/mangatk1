...
import { ChapterData } from '@/data/mockChapters';
...
export default function ReaderPage() {
  ...
  useEffect(() => {
    const fetchImages = async () => {
      setLoading(true);
      try {
        const response = await fetch(`/api/get-chapter-images?chapterId=${chapterId}`);
        const data = await response.json();

        if (data.success) {
          const currentManga = mockManga.find(m => m.id === mangaId);
          const coverImage = currentManga ? currentManga.imageUrl : (data.images[0]?.url || '');
          const mangaTitle = currentManga ? currentManga.title : "Unknown Manga";
  ...
  }, [chapterId, mangaId, addToHistory]);
      ...
      {mode === 'vertical' && (
        ...
        <div className="w-full px-4 mt-12">
            ...
            <ChapterRating 
              ...
              currentMangaRating={mockManga.find(m => m.id === mangaId)?.avgRating || 4.5} 
            />
        </div>
